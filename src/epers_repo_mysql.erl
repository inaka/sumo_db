...
persist(#epers_doc{name=DocName}=Doc, State) ->
  ...
  IdField = epers:field_name(epers:get_id_field(DocName)),
  {ok, epers:set_field(IdField, LastId, Doc), State}.

delete(DocName, Id, State) ->
  Sql = "DELETE FROM `" ++ atom_to_list(DocName)
    ++ "` WHERE `" ++ atom_to_list(epers:field_name(epers:get_id_field(DocName)))
    ++ "`=? LIMIT 1",
    ...
    fun(Row, DocList) ->
      NewDoc = lists:foldl(
        fun(Field, [Doc,N]) ->
          ...
          [epers:set_field(FieldName, Field, Doc), N+1]
        end,
        [epers:new_doc(DocName), 1],
        Row
      ),
      [hd(NewDoc)|DocList]
    end,
...
create_column(#epers_field{name=Name, type=integer, attrs=Attrs}) ->
  "`" ++ atom_to_list(Name) ++ "` INT(11) " ++ create_column_options(Attrs);

create_column(#epers_field{name=Name, type=text, attrs=Attrs}) ->
  "`" ++ atom_to_list(Name) ++ "` TEXT " ++ create_column_options(Attrs);

create_column(#epers_field{name=Name, type=binary, attrs=Attrs}) ->
  "`" ++ atom_to_list(Name) ++ "` BLOB " ++ create_column_options(Attrs);

create_column(#epers_field{name=Name, type=string, attrs=Attrs}) ->
...
create_index(#epers_field{name=Name, attrs=Attrs}) ->
  lists:filter(fun(T) -> is_list(T) end, lists:map(
    fun(Attr) ->
