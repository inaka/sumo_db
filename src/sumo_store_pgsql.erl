...
execute(Name, Args, #{conn := Conn}) when is_atom(Name), is_list(Args) ->
  {Time, Value} = timer:tc( emysql, execute, [Conn, Name, Args] ),
...
execute(Name, State) when is_atom(Name) ->
  execute(Name, [], State);
execute(PreQuery, #{conn := Conn}) when is_list(PreQuery)->
  Query = iolist_to_binary(PreQuery),
  {Time, Value} = timer:tc( emysql, execute, [Conn, Query] ),
