...
%% repository name.
-spec find_by(
  atom(), epers_schema_name(), proplists:proplist(),
...
-spec call(atom(), epers_schema_name(), atom(), [term()]) -> term().
...
-spec create_schema(atom(), #epers_schema{}) -> ok.
create_schema(Name, #epers_schema{}=Schema) ->
...
handle_call(
  {persist, #epers_doc{name=DocName}=Doc}, _From,
  #state{handler=Handler,handler_state=HState}=State
) ->
  {ok, DocState, NewState} = Handler:persist(Doc, HState),
  {reply, DocName:epers_wakeup(DocState), State#state{handler_state=NewState}};
...
handle_call(
  {create_schema, #epers_schema{name=Name}=Schema}, _From,
  #state{handler=Handler,handler_state=HState}=State
) ->
